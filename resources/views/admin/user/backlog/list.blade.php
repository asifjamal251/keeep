      ...
      <div class="btn-group float-md-right" role="group" aria-label="Button group with nested dropdown">
        <div class="btn-group" role="group">
            <div class="form-group{{ $errors->has('status') ? ' has-error' : '' }}">
                {!! Form::select('status', [10=>'Processed',9=>'Unprocess',13=>'Assign'], null, ['id' => 'status', 'class' => 'form-control', 'placeholder' => 'Select Status','onChange'=>'status(this)']) !!}
...
<div class="card">
    <div class="card-content">
         <div class="card-header" id="heading-links">
            <span class="card-title pull-left" style="text-transform: capitalize;"><b>Name:</b> {{$user->name}}</span>
            <span class="card-title pull-right" style="text-transform: lowercase;"><b style="text-transform: capitalize;">Email:</b>{{$user->email}}</span>
...
var selectedRows = new Array
  ...
  //alert('hello');  
            ...
            var table2 = $('#dataTableAjax').DataTable({
                ...
                "serverSide": true,
                'ajax': {
                    'url': '{{ route('admin.backlog.user.image',$user) }}',
                    'data': function(d) {
                        d._token = '{{ csrf_token() }}';
                        d._method = 'PATCH';
                        d.status = $('#status').val();
                        ...
                        render: function(data, type, row) {
                            if (type === 'display') {
                                ...
                                if( row['changeStatus']=='10' || row['changeStatus']=='4'){
                                    btn+='<button type="button" onclick="updateData(\'{{ route('admin.keep.changeStatus') }}\',{id:'+row['id']+',status:14})" class="btn btn-social-icon btn-outline-primary btn-xs" data-toggle="tooltip" data-placement="bottom" title="" data-original-title="Reassign"><i class="ft-corner-right-up"></i></button> ';

                                    btn+='<button type="button" onclick="updateData(\'{{ route('admin.keep.changeStatus') }}\',{id:'+row['id']+',status:6})" class="btn btn-social-icon btn-outline-success btn-xs" data-toggle="tooltip" data-placement="bottom" title="" data-original-title="Approve"><i class="ft-check-square"></i></button> ';
